...
const renderComponent = (savedVideosList: VideoDetails[], isDarkTheme: boolean) => {
    const value = {
      isDarkTheme,
      toggleTheme: () => {}, 
      save: false,
  ...
  it('should toggle between light and dark theme', () => {
    ...
    rerender(
      <ThemeContext.Provider value={{ isDarkTheme: true, toggleTheme: () => {} }}>
        <SavedVideosContext.Provider value={{ save: false, savedVideosList: [mockSavedVideo], addVideosToSavedVideos: () => {}, deleteVideosFromSavedVideos: () => {}, updateSave: () => {} }}>
          <MemoryRouter>
            <SavedVideos />
          </MemoryRouter>
        </SavedVideosContext.Provider>
