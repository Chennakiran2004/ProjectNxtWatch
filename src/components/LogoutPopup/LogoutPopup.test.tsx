...
describe("LogoutPopup", () => {
  ...
  it("should apply correct styles based on props in Button component", () => {
    const { container } = render(<Button color="red" outline={true}>Test Button</Button>);
    ...
    const { container: nonOutlinedContainer } = render(<Button color="green">Test Button</Button>);
    const nonOutlinedButtonElement = nonOutlinedContainer.querySelector('button');
  ...
  it("should display popup with correct styles based on theme", () => {
    const { getByText } = render(<LogoutPopup onClickLogout={mockOnClickLogout} theme="light" />);
    
    const logoutButton = screen.getByTestId("Logout");
    fireEvent.click(logoutButton);

    const popupContent = screen.getByText(/Are you sure, you want to logout?/i).closest('div');
